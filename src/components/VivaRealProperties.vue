            ...
            axios.get('http://grupozap-code-challenge.s3-website-us-east-1.amazonaws.com/sources/source-1.json')
                .then(res => {
                    ...
                    for (let key in data) {
                        ...
                        if (data[key].pricingInfos.businessType == "RENTAL" && data[key].pricingInfos.price >= 4000) {
                            this.propertys.push(property);
                        }
                        if (data[key].pricingInfos.businessType == "SALE" && data[key].pricingInfos.price >= 700000) {
                            this.propertys.push(property);
                        }
                    }
                })
